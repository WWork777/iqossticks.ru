...
export async function POST(request) {
  try {
    ...
    const currentOrderNumber = previousCount + 1;
    ...
    for (const item of items) {
      ...
      // Форматируем строку товара как в вашем примере
      itemsText += `- ${item.name} (${item.type || "обычный"}) x${item.quantity}: ${item.price * item.quantity} ₽\n`;
    }

    // 4. Формируем заголовок и переменные
    const headerStatus = isFirst
      ? "🔥 НОВЫЙ КЛИЕНТ 🔥"
      : `📋 Повторный заказ (${currentOrderNumber}-й по счету)`;

    const tgName = orderData.telegram_username
      ? orderData.telegram_username.startsWith("@")
        ? orderData.telegram_username
        : `@${orderData.telegram_username}`
      : "не указан";

    // Определяем название сайта (берем из orderData.website, который прислал фронтенд)
    const siteDisplayName = orderData.website || "iqossticks.ru";

    // 5. Итоговое сообщение в Telegram (строго по вашему формату)
    const message = `Заказ с сайта ${siteDisplayName}
